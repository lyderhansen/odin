# TA-ODIN v1.0.2 - Windows Orchestrator
#
...
if (-not (Test-Path $CommonLib)) {
    ...
    Write-Output "timestamp=$ts hostname=unknown os=windows run_id=error-$PID odin_version=1.0.2 type=odin_error message=`"_common.ps1 not found at $CommonLib`""
    exit 1
}
