#!/bin/bash
#
# TA-ODIN v1.0.2 - Orchestrator Script for Linux
...
if [[ -z "$BASH_VERSION" ]]; then
    echo "timestamp=$(date -u +"%Y-%m-%dT%H:%M:%SZ") hostname=$(hostname) os=linux run_id=error-$$ odin_version=1.0.2 type=odin_error message=\"TA-ODIN requires bash but it is not available on this system\""
    exit 1
fi
